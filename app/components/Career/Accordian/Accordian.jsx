...
const Accordion = ({ openFormModule }) => {
  const contentRefs = useRef([]); // Reference to measure height
  ...
  return (
    <div className={accordianStyles.accordion_container}>
      {accordionData.map((item, index) => (
        <div key={index} className={accordianStyles.accordion_item}>
          ...
          {/* <div className={`${accordianStyles.set_content} ${activeIndex === index ? accordianStyles.active : ''}`} >{item.content}</div> */}
          <div
            ref={(el) => (contentRefs.current[index] = el)}
            className={`${accordianStyles.set_content} ${activeIndex === index ? accordianStyles.active : ""
              }`}
            style={{
